...
public class UpdateParkingCommand implements Command{
	...
	public Router execute(HttpServletRequest request) {
		...
		try {
			String errorMessage = parkingService.updateParking(requestParameters);
			
			if(errorMessage.isEmpty()) {
				router.setPagePath(PageConstant.REDIRECT_TO_PARKING_PAGE);
				HttpSession session = request.getSession(true);
				session.setAttribute(RequestParameter.MESSAGE.parameter(), MessagePage.PARKING_CHANGED.message());
			}else {
